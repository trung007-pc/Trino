...
public class TrinoDbSet<T> : IQueryable<T>, IEnumerable<T> where T : class, new()
{
    ...
    /// <summary>
    /// Project entities sang type mới với lambda expression - giống Select() trong LINQ
    /// Type-safe and SQL injection protected
    /// Usage: var query = context.Customers.Select(c => new { c.Name, c.Email });
    ///        var results = await query.ToListAsync(); // Execute here
    /// </summary>
    public TrinoProjection<T, TResult> Select<TResult>(Expression<Func<T, TResult>> selector)
    {
        // Lazy evaluation - chưa execute query
        return new TrinoProjection<T, TResult>(
            _connection, 
            _tableName, 
            selector,
            _whereConditions,
